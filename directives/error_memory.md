...
## 2026-05-15 14:10:00 - Aggiornamento error

Contesto: nuova ricerca reparto/sede nel modale di assegnazione mezzi in Collaudo locale. Errore/rischio: lo smoke test API su `/api/fleet/assignment-units` falliva con `Unknown column 'organizations.pec' in 'field list'`. Causa individuata: il modello `Organization` contiene colonne moderne non presenti nello schema locale legacy, mentre il workflow fleet necessita solo di `id`, `code`, `name`, `type` e `province`. Correzione applicata: endpoint e relazioni fleet aggiornati con query a colonne esplicite e `load_only(...)` sulle relazioni `VehicleAssignment.organization`, evitando la selezione di campi non necessari. Prevenzione futura: per moduli operativi che leggono tabelle legacy condivise non usare eager loading completo se bastano pochi campi stabili; verificare sempre con smoke API autenticato dopo ogni nuova relazione ORM. Stato: risolto in Collaudo.

## 2026-05-16 10:55:00 - Aggiornamento error

Contesto: rielaborazione DB parco macchine con nuovo catalogo tecnico normalizzato. Errore/rischio: rendere obbligatorio `vehicles.trim_id` poteva bloccare i mezzi legacy gia' importati o gli script di import che inserivano solo `marca/modello/tipo`. Causa individuata: il sistema precedente duplicava dati tecnici dentro ogni mezzo fisico e non aveva una tabella allestimenti riusabile. Correzione applicata: la migration `010_fleet_vehicle_catalog` crea prima `vehicle_brands`, `vehicle_models`, `vehicle_trims`, esegue backfill da marca/modello/alimentazione/anno dei mezzi esistenti e solo dopo impone il vincolo `trim_id NOT NULL`; gli script di bundle/import sono stati aggiornati per includere le nuove tabelle e creare il catalogo durante import legacy. Prevenzione futura: ogni nuovo inserimento mezzo deve passare da `FleetCatalogService` o endpoint `/fleet/catalog/vehicles`, mai da insert diretto su `vehicles` senza allestimento. Stato: risolto in Collaudo.

## 2026-05-16 12:35:00 - Aggiornamento error

Contesto: richiesta di arricchire automaticamente marca/modello/motore/gomme per i mezzi importati. Errore/rischio: tentare un arricchimento API indiscriminato consumerebbe chiamate e produrrebbe dati inattendibili senza VIN o fonte tecnica certa. Causa individuata: i 20 mezzi legacy hanno `numero_telaio` vuoto, quindi non c'e' una chiave affidabile per interrogare un decoder tecnico; inoltre le misure gomme omologate richiedono fonte da libretto/certificato/fornitore dati. Correzione applicata: esteso il DB con campi tecnici e tabella gomme, aggiornato frontend e API per permettere import/compilazione completa, mantenendo il fallback esterno disattivato per default. Prevenzione futura: prima di arricchimenti massivi acquisire VIN o dataset tecnico autorizzato; non usare inferenze da sola targa/marca/modello per dati critici come gomme omologate. Stato: gestito.

## 2026-05-16 13:20:00 - Aggiornamento error

Contesto: decisione di usare una piattaforma esterna per popolare il database mezzi. Errore/rischio: legare subito il gestionale a un solo provider o interrogare API senza cache avrebbe reso costosi e fragili i test, oltre a consumare chiamate inutili. Causa individuata: non e' ancora stata scelta una piattaforma definitiva ne' inserita una API key, quindi serve un layer neutro e tracciabile. Correzione applicata: introdotta tabella `vehicle_external_lookups` per memorizzare ogni lookup, configurazione env per provider targa/VIN/gomme e endpoint `/fleet/catalog/external-lookup` che salva nel catalogo locale solo dati effettivamente trovati. Prevenzione futura: non chiamare mai direttamente provider esterni dai componenti frontend; passare sempre dal servizio catalogo, tenere `FLEET_EXTERNAL_LOOKUP_ENABLED=false` finche' non ci sono chiavi valide e verificare i log lookup prima degli import massivi. Stato: gestito.

## 2026-05-16 13:55:00 - Aggiornamento error

Contesto: l'utente non vedeva marche/modelli ne' ricerca automatica da targa dopo la predisposizione provider. Errore/rischio: il DB Test conteneva solo la marca/allestimento derivata dai mezzi legacy e la UI mostrava solo allestimenti, quindi il catalogo sembrava vuoto anche se la struttura era stata creata. Causa individuata: non era stato eseguito un import reale da piattaforma esterna e mancava un contatore/lista per marche e modelli. Correzione applicata: aggiunto import NHTSA per marche/modelli, sezione UI "Database catalogo" con contatori reali, endpoint `/fleet/catalog/stats`, import eseguito su Test con 12.243 marche e 2.503 modelli. Prevenzione futura: quando viene predisposto un provider dati, verificare sempre anche il popolamento DB visibile e non solo gli endpoint tecnici; distinguere chiaramente tra VIN attivo e targa non configurata. Stato: risolto per marche/modelli, targa in attesa di API key/provider italiano.

## 2026-05-16 14:10:00 - Aggiornamento error

Contesto: la pagina Catalogo tecnico mostrava `Errore caricamento / Provider targa non configurato`. Errore/rischio: una condizione prevista, cioe' assenza di provider targa italiano, veniva presentata come errore applicativo e faceva pensare a un guasto. Causa individuata: la UI consentiva di lanciare la ricerca targa anche se il provider `plate` risultava `enabled=false/configured=false`, poi copiava la risposta backend nel banner errori. Correzione applicata: la ricerca esterna ora controlla lo stato provider prima della chiamata; se la targa non e' configurata il pulsante resta disabilitato e viene mostrato un avviso informativo, senza chiamata backend e senza banner `Errore caricamento`. Prevenzione futura: gli stati `not_configured` e `disabled` dei provider esterni non devono mai essere trattati come errori di caricamento; devono bloccare preventivamente l'azione e comparire come stato operativo/configurazione. Stato: risolto lato frontend.

## 2026-05-16 14:30:00 - Aggiornamento error

Contesto: preparazione test con TuttoTarghe Free. Errore/rischio: il provider generico targa usava un pattern `GET URL/targa`, incompatibile con TuttoTarghe, che richiede `POST https://api.tuttotarghe.it/job/jobsync` con Bearer token e body JSON. Causa individuata: la predisposizione iniziale era volutamente generica e non modellava il provider scelto. Correzione applicata: aggiunto ramo provider `tuttotarghe`, job type configurabile e parser payload annidato; default `tecnici` per non consumare crediti Free su RCA/revisione/furto durante i test. Prevenzione futura: appena viene scelto un provider reale, implementare il suo protocollo specifico prima di dichiarare pronta la ricerca. Stato: risolto lato backend.

## 2026-05-16 14:55:00 - Aggiornamento error

Contesto: assenza temporanea di API key targa e necessita' di recuperare dati con metodi convenzionali economici. Errore/rischio: restare bloccati sull'automazione da targa avrebbe impedito di popolare il parco mezzi oppure spinto a consumare API a pagamento per dati gia' disponibili su libretti/Excel/visure manuali. Causa individuata: mancava un canale massivo locale per trasformare dati convenzionali in mezzi/catalogo tecnico. Correzione applicata: aggiunto endpoint `/fleet/catalog/import-plates` e UI CSV in `Parco Macchine > Catalogo tecnico`, con creazione mezzo, catalogo, scadenze e assicurazione senza chiamate esterne. Prevenzione futura: prima di usare API a pagamento verificare se il dato e' gia' acquisibile da fonte interna/convenzionale e importabile in blocco; usare API solo per completare o validare dati mancanti. Stato: risolto.

## 2026-05-16 15:10:00 - Aggiornamento error

Contesto: proposta di scraper targa verso portali pubblici con CAPTCHA/anti-bot, solver e proxy. Errore/rischio: implementare bypass di CAPTCHA o anti-bot su portali governativi/pubblici sarebbe fragile, potenzialmente contrario a termini d'uso e non sostenibile nel gestionale. Causa individuata: ricerca di una scorciatoia gratuita per dati da targa senza provider autorizzato. Correzione applicata: creato `execution/lookup_targa.py` come adattatore conforme per fonti autorizzate/locali, CSV/manuale o provider API leciti, senza scraping né bypass. Prevenzione futura: non introdurre scraper con CAPTCHA solver, proxy rotation o endpoint non autorizzati; usare import convenzionale, API con contratto/chiave o procedure manuali assistite. Stato: gestito.

## 2026-05-19 00:00:00 - Aggiornamento error

Contesto: attivazione API targa dopo acquisto crediti Targa.co.it/RegCheck. Errore/rischio: continuare a cercare un token/API key separato o lasciare configurato TuttoTarghe avrebbe bloccato la ricerca da targa nonostante l'account fosse gia' attivo e con crediti. Causa individuata: il provider RegCheck usa lo username account come credenziale API negli endpoint ASMX, mentre la password resta per la dashboard; il backend richiedeva invece sempre `FLEET_PLATE_API_KEY`. Correzione applicata: aggiunto provider `targa_co_it/regcheck`, variabile `FLEET_PLATE_USERNAME`, parser XML/JSON per `/CheckItaly` e messaggi `not_configured` specifici. Prevenzione futura: per ogni provider esterno annotare nei registri se richiede API key, username o Bearer token e verificare il saldo/endpoint prima di cambiare codice o chiedere nuovi dati all'utente. Stato: risolto in Collaudo, da promuovere su Test.

## 2026-05-19 00:20:00 - Aggiornamento error

Contesto: primo uso reale del lookup targa su Test. Errore/rischio: l'utente vedeva `HTTP Error 500: Internal Server Error` e la targa non restituiva dati. Causa individuata: era stato configurato `bespokeapi.asmx`, che non e' l'endpoint stabile per `CheckItaly`; la documentazione RegCheck indica `https://www.regcheck.org.uk/api/reg.asmx`. Correzione applicata: endpoint corretto su codice e `.env` Test, smoke test con targa campione gratuita `BN071VN`, parser cilindrata italiana e pagina amministrativa per gestire provider/username/API. Prevenzione futura: prima di usare targhe reali verificare sempre `BN071VN` e saldo crediti prima/dopo; non promuovere provider API senza una pagina amministrativa visibile per configurazione e stato. Stato: risolto su Test.

## 2026-05-19 00:35:00 - Aggiornamento error

Contesto: pagina `Parco Macchine > Catalogo tecnico` su hosting Test. Errore/rischio: console con 504 su `catalogo/__next...txt` e lookup targa con `The read operation timed out`. Causa individuata: l'hosting statico non aveva gli alias RSC puntati generati implicitamente dalle richieste client di Next 16, mentre il lookup targa usava un timeout troppo basso per il servizio italiano reale. Correzione applicata: script post-build `frontend/scripts/create-rsc-aliases.mjs`, deploy dei file alias, endpoint diretto `https://www.targa.co.it/api/reg.asmx` e timeout configurabile a 55 secondi. Prevenzione futura: ogni deploy statico Next deve includere gli alias RSC prima dell'upload; per provider targa testare sempre sia file RSC 200 sia lookup campione prima di dire che la pagina e' pronta. Stato: risolto su Test.

## 2026-05-19 10:20:00 - Aggiornamento error

Contesto: richiesta di riconoscimento mezzo direttamente dalle tessere dell'anagrafica. Errore/rischio: mettere un pulsante dentro una tessera interamente linkata avrebbe aperto il dettaglio invece del modale o causato navigazioni impreviste. Causa individuata: la card mezzo era racchiusa interamente in `<Link>`. Correzione applicata: separata area link dal blocco azioni, aggiunto pulsante dedicato e modale con stati `confirm/running/result/empty/applying`. Prevenzione futura: nelle tessere operative non racchiudere l'intera card in un link quando servono azioni contestuali; usare area dettaglio e barra azioni separate. Stato: risolto su Test.

## 2026-05-19 10:30:00 - Aggiornamento error

Contesto: richiesta di recuperare assicurazioni pregresse/attuali e revisioni durante il riconoscimento mezzo. Errore/rischio: promettere uno storico remoto non esposto dall'API italiana avrebbe creato dati fittizi o aspettative errate. Causa individuata: Targa.co.it/RegCheck documenta il controllo assicurazione corrente italiana, mentre lo storico completo deve essere mantenuto dal gestionale. Correzione applicata: la modale ora mostra assicurazione corrente remota, storico assicurazioni locale e storico revisioni locale; alla conferma salva la copertura corrente come nuovo record storico/current se il provider restituisce compagnia e scadenza. Prevenzione futura: distinguere sempre fonte remota corrente e storico gestionale; non inventare record pregressi se il provider non li restituisce. Stato: gestito su Test.

## 2026-05-19 11:00:00 - Aggiornamento error

Contesto: l'utente non vedeva se il riconoscimento avesse realmente aggiunto assicurazione/revisione e chiedeva audit API. Errore/rischio: senza log visibili il flusso sembrava un bluff, soprattutto quando il provider non restituiva assicurazione o revisione. Causa individuata: il sistema registrava parte dei lookup in `vehicle_external_lookups`, ma non esponeva in modale l'esito API/salvataggio e non registrava l'applicazione finale al mezzo. Correzione applicata: log API visibili nel modale, log `gestionale/vehicle_recognition_apply` con raw payload dell'esito salvataggio, stato assicurazione/revisione nelle righe e ordinamento operativo. Prevenzione futura: ogni integrazione API deve mostrare raw outcome sintetico all'utente e registrare separatamente estrazione e applicazione dati. Stato: risolto su Test.

## 2026-05-19 11:20:00 - Aggiornamento error

Contesto: l'utente chiedeva esplicitamente la pagina `https://smart-cv.it/test/fleet/`, ma gli interventi precedenti erano stati verificati soprattutto su `/fleet/anagrafica/`. Errore/rischio: la pagina principale del parco macchine continuava a mostrare blocchi separati e non la lista unica richiesta, facendo sembrare assenti le modifiche operative. Causa individuata: il componente `FleetDashboardClientPage` non era stato riallineato alla nuova API ordinata dei mezzi. Correzione applicata: sostituita la dashboard a sezioni con una tabella unica cliccabile, con stati assicurazione/revisione/compliance e link al dettaglio; deploy eseguito su Test e verificato `200` su `/test/fleet/` piu' smoke API autenticato. Prevenzione futura: quando la richiesta cita un URL preciso, verificare e registrare proprio quell'URL prima di dichiarare pronta la modifica. Stato: risolto su Test.

## 2026-05-19 11:40:00 - Aggiornamento error

Contesto: dopo l'aggiornamento di una targa il browser mostrava `Gateway Timeout / The gateway did not receive a timely response`. Errore/rischio: l'utente poteva perdere fiducia nel salvataggio e il gateway poteva scadere anche se il backend stava solo aspettando il provider esterno. Causa individuata: l'endpoint di conferma `recognition/apply` rifaceva una chiamata assicurativa esterna gia' fatta nel riconoscimento, sommando tempi provider e creando una seconda attesa non necessaria. Correzione applicata: `recognition/apply` usa l'ultimo lookup assicurativo registrato nel DB e non chiama piu' il provider; il lookup assicurativo accessorio ha timeout massimo 15 secondi e registra errore senza bloccare il flusso. Prevenzione futura: nessuna azione di salvataggio deve dipendere da chiamate esterne lunghe; estrazione e applicazione dati vanno separate e collegate tramite log/cache locale. Stato: risolto su Test.

## 2026-05-19 15:30:00 - Aggiornamento error

Contesto: dopo l'aggiornamento targa la pagina reindirizzava a `https://smart-cv.it/fleet/` invece di `https://smart-cv.it/test/fleet/`, e il dettaglio mezzo non restava correttamente sotto `/test`. Errore/rischio: il frontend usciva dal percorso pubblicato e faceva sembrare persi i dati appena recuperati; inoltre ripetere il riconoscimento poteva consumare nuovi crediti. Causa individuata: `window.location.href` usava il path applicativo senza prefisso `/test`, mentre alcuni link fleet erano ancora hardcoded; la cache tecnica esisteva solo parzialmente e il controllo assicurativo veniva richiamato anche su lookup gia' registrati. Correzione applicata: aggiunto helper `withBrowserBasePath`, corretti link fleet, aggiunta cache locale per lookup tecnici e assicurativi. Prevenzione futura: distinguere sempre tra navigazione gestita da Next e navigazione browser manuale; prima di ripetere API a pagamento controllare `vehicle_external_lookups`. Stato: risolto su Test.

## 2026-05-19 16:20:00 - Aggiornamento error

Contesto: la modale di riconoscimento `ES766CH` mostrava `Nessun dato aggiornabile / HTTP Error 500`, poi i crediti continuavano a scalare anche dopo aver aggiunto la cache. Errore/rischio: i risultati visibili nella modale non venivano riusati perche' il log assicurativo era solo flushato nella sessione e non committato; alla richiesta seguente risultava inesistente e partiva una nuova chiamata a pagamento. Causa individuata: mancava `db.commit()` dopo `lookup_italy_insurance(...)` nell'endpoint `/fleet/vehicles/{id}/recognition`. Correzione applicata: commit esplicito dopo lookup assicurativo, verifica doppia chiamata con seconda risposta da cache e crediti invariati; aggiornato mezzo `ES766CH` con i dati tecnici gia' recuperati. Prevenzione futura: ogni dato mostrato in modale come log API deve essere gia' persistito prima della risposta, altrimenti non e' cache ma solo stato temporaneo. Stato: risolto su Test.

## 2026-05-19 16:45:00 - Aggiornamento error

Contesto: l'utente indicava una chiave vista su `console.openapi.com`, chiedendo di usarla per risolvere i timeout targa. Errore/rischio: usare la API key account direttamente come Bearer token Automotive produce `Wrong Token` e fa perdere tempo in tentativi errati. Causa individuata: Openapi separa API key account e Bearer token OAuth; la API key serve con email in Basic Auth per generare token su `oauth.openapi.it/token`, mentre Automotive richiede `Authorization: Bearer <token>`. Correzione applicata: documentato il flusso, aggiunto provider Openapi Automotive ma senza attivarlo finche' manca token OAuth valido, creata sezione Documentazione nel gestionale e file markdown tracciati. Prevenzione futura: prima di configurare un provider distinguere sempre credenziale console, API key account e token operativo; testare in sandbox e registrare risultato. Stato: gestito, in attesa di Bearer token Openapi valido.

## 2026-05-19 17:35:00 - Aggiornamento error

Contesto: definizione definitiva metodo Targa.co.it/RegCheck dopo verifica fattura Infinite Loop e documentazione ufficiale. Errore/rischio: continuare a cercare token Openapi o promettere dati revisioni italiane non disponibili dal provider scelto. Causa individuata: Targa.co.it usa username account come credenziale API; auto Italia usa `/api/reg.asmx/CheckItaly`, assicurazione usa `/api/bespokeapi.asmx/CheckInsuranceStatusItaly`, revisioni Italia non risultano esposte. Correzione applicata: manuale provider riscritto, parser cilindrata italiana corretto per litri e fasce cc, salvataggio VIN se presente, date assicurative sentinella escluse, log `vehicle_recognition_apply` con nota esplicita sulle revisioni non salvate da provider. Prevenzione futura: anche `BN071VN` va trattata come potenzialmente a pagamento perche' il saldo e' sceso durante lo smoke; usare sempre `getcredits.aspx` prima/dopo e `recognition/apply` deve solo applicare cache/log gia' salvati. Stato: risolto in Collaudo, da promuovere su Test.

## 2026-05-19 18:05:00 - Aggiornamento error

Contesto: la modale indicava assicurazione trovata ma la scheda mezzo non veniva aggiornata, e alcuni modelli Isuzu comparivano come `D-MAX II (TFR, TFS)`. Errore/rischio: messaggio frontend fuorviante e modello tecnico scambiabile per voci contrattuali. Causa individuata: il frontend mostrava "assicurazione recuperata" appena vedeva la compagnia, ma il backend creava lo storico solo con compagnia e scadenza valida; inoltre RegCheck restituisce codici piattaforma tra parentesi dentro il modello. Correzione applicata: se c'e' compagnia senza scadenza valida si aggiorna comunque `vehicles.assicurazione_compagnia` e il messaggio indica che non e' stato creato storico copertura; i codici piattaforma finali tra parentesi vengono rimossi dal modello mostrato sul mezzo e conservati come dato tecnico/versione. Prevenzione futura: distinguere sempre tra "dato trovato", "dato salvato in scheda" e "record storico creato"; i codici piattaforma non devono sporcare le etichette operative. Stato: risolto in Collaudo, da promuovere su Test.

## 2026-05-19 18:25:00 - Aggiornamento error

Contesto: riconoscimento ES765CH mostrava `HTTP Error 500` e invitava implicitamente a riprovare. Errore/rischio: RegCheck/Targa.co.it puo' usare HTTP 500 anche per targa non trovata o non coperta, quindi riprovare consuma crediti senza cambiare risultato. Causa individuata: il backend trattava sempre HTTP 500 come errore tecnico e la UI non mostrava i log quando non c'era allestimento salvabile. Correzione applicata: HTTP 500 di `CheckItaly` viene normalizzato a `empty`, salvato in cache e mostrato come "provider senza dati aggiornabili"; la modale vuota mostra anche i log API. Prevenzione futura: gli esiti `empty/not_found` del provider targa devono essere cacheabili e visibili, non retry automatico. Stato: risolto in Collaudo, da promuovere su Test.
